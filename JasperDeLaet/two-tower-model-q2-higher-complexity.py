...
class LabelEncoder:
    def __init__(self, labels):
        # Add the unknown label to the set of labels
...
train_df = transactions_df[(transactions_df['t_dat'] >= '2020-09-01') & (transactions_df['t_dat'] <= '2020-09-14')]
...
print(len(train_df['article_id'].unique()))

print(customer_encoder.vocab_size)
print(article_encoder.vocab_size)
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        return customer_id, positive_article_id, negative_article_id
...
class TwoTower(nn.Module):
    ...
    def __init__(self, n_users, n_items):
        ...
        self.user_layers = nn.Sequential(
            ...
            nn.Linear(128, 128, bias=True),
            nn.LeakyReLU(),
            nn.Linear(128, embed_dim, bias=True),
            nn.LeakyReLU()
        )
...
def train(model, device, dataloader, optimizer):
    ...
    for batchidx, data in enumerate(dataloader):
        ...
        positive_predictions = model(customers, positive_articles)
        negative_predictions = model(customers, negative_articles)
