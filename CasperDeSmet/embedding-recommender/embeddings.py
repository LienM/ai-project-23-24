...
def create_image_embeddings(image_directory, output_file):
    if os.path.exists(output_file):
        return pandas.read_parquet(output_file)

    data = load_data(image_directory, BATCH_SIZE)
...
def create_text_embeddings(articles_file, output_file, template="plain"):
    if os.path.exists(output_file):
        return pandas.read_parquet(output_file)

    bert = pipeline(task="feature-extraction", model="bert-base-uncased", device=device)

    with open("embedding-recommender/text_embeddings.json", "r") as config_file:
        config = json.load(config_file)
        columns = config["columns"]
        templates = config["templates"]

    # Apply template to article text
    articles = pandas.read_csv(articles_file, dtype=str).fillna(value="")
...
def concatenate_embeddings(text_embeddings_file, image_embeddings_file, output_file):
    if os.path.exists(output_file):
        return pandas.read_parquet(output_file)
    
    text_embeddings = pandas.read_parquet(text_embeddings_file)
    image_embeddings = pandas.read_parquet(image_embeddings_file)
    ...
    concatenated_embeddings.to_parquet(output_file)
