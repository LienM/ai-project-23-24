...
# Describe data
def describe_data(df: pd.DataFrame, name: str, output_path_: pathlib.Path, thread_start_time: float = None) -> None:
...
def describe_images(path: pathlib.Path, output_path_: pathlib.Path, mp_pool_count_: int = 1) -> None:
    time_start = time.time()
    output_file = output_path_ / 'images_description.md'
    ...
    with mp.Pool(processes=mp_pool_count_) as pool_:
        df = df._append(pool_.map(add_image_info, images), ignore_index=True)
...
def create_plot(df: pd.DataFrame, name: str, output_path_: pathlib.Path, combine_smallest: float = 0,
                binwidth: float = 2, height: int = 5,
                aspect: int = 2, plot_width: int = 15, plot_height: int = 10, rotation: float = 60, dpi: int = 400,
                **kwargs) -> None:
    print(f'[ ] Creating plot for {name}...')
    df = df.copy()

    if combine_smallest > 0:
        # TODO: Fix this. Seems to replace a random value with the "Other" value.
        total_count = df[name].count()
        counts = df[name].value_counts().sort_values(ascending=False)
        current_amount = 0
        for index, count in counts.items():
            if count / total_count > combine_smallest:
                break
            current_amount += count
            if current_amount / total_count < combine_smallest:
                df[name] = df[name].replace(index, f'Other ({combine_smallest * 100:.0f}%)')
            else:
                break
    ...
    plt.tight_layout()
    plt.savefig(output_path_ / f'{name}.png', dpi=dpi, bbox_inches='tight', **kwargs)
    plt.close()
...
if __name__ == '__main__':
    ...
    # TODO: slower than single thread, presumably due to the overhead of multiprocessing and passing data between processes
    # with mp.Pool(processes=mp_pool_count) as pool:
    #     articles, customers, sample_submission, transactions_train = pool.map(load_data,
    #                                                                           [articles_path, customers_path,
    ...
    with mp.Pool(processes=mp_pool_count) as pool:
        pool.starmap(create_plot, [(customers, 'age', plot_path),
    ...
    with mp.Pool(processes=mp_pool_count) as pool:
        pool.starmap(describe_data, [(articles, 'articles', output_path, thread_start_time),
    ...
    # Describe images
    describe_images(images_path, output_path, mp_pool_count)
