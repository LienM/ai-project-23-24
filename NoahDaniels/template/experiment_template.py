...
def get_purchases(transactions):
    """
    Convert a dataframe containing transactions to a dataframe where each row has a customer_id and a set of purchases for that customer.
...
def fill_missing_predictions(predictions, customers, prediction):
    ...
    return pd.concat((predictions, missing_predictions)).reset_index(drop=True)
...
def mean_average_precision(predictions, purchases, k=12):
    ...
    Each row in the predictions and purchases has a customer_id and a set of purchases and list of predictions respectively.
