...
class LastFM(BasicDataset):
    ...
    def __init__(self, path="../input"):
        ...
        # print(trustNet[:5])
        # trustNet -= 1
        # trainData, testData = self.getTrainAndTest()
        ...
        self.trainUser = np.array(trainData.iloc[:, 0])
        self.trainUniqueUsers = np.unique(self.trainUser)
        self.trainItem = np.array(trainData.iloc[:, 1])
        # self.trainDataSize = len(self.trainUser)
        self.testUser = np.array(testData.iloc[:, 0])
        self.testUniqueUsers = np.unique(self.testUser)
        self.testItem = np.array(testData.iloc[:, 1])
        self.Graph = None
    ...
    def getSparseGraph(self):
        if self.Graph is None:
            ...
            # D = torch.sparse.sum(self.Graph, dim=1).float()
            # D[D == 0.] = 1.
            # D_sqrt = torch.sqrt(D).unsqueeze(dim=0)
            # self.Graph = self.Graph / D_sqrt
            # self.Graph = self.Graph / D_sqrt.t()
            # index = self.Graph.nonzero()
            # data = self.Graph[self.Graph >= 1e-9]
