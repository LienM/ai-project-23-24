...
#Modified by Pablo de Vicente. NOT original FILE!!
...
class Pipeline(object):
    ...
    def run2(self):
        """Runs the pipeline."""
        X_pred = self.run_algorithms()
        return X_pred
    
    def run_algorithms(self):
        """Runs the algorithms in the pipeline."""
        #we will only test one at a time because i dont want to redo this things
        for algorithm_entry in tqdm(self.algorithm_entries):
            params = self._get_algorithm_params(algorithm_entry)
            algorithm = self._initialize_algorithm(algorithm_entry, params)
            self._train_algorithm(algorithm)
            X_pred = self._predict_and_postprocess_data(algorithm)
            # i dont really care abt the metrics
            # self.evaluate_metrics(algorithm, X_pred)
            return X_pred

    def _get_algorithm_params(self, algorithm_entry):
        """Gets the parameters for the algorithm."""
        if algorithm_entry.optimise:
            return self._optimise_hyperparameters(algorithm_entry)
        else:
            return algorithm_entry.params

    def _initialize_algorithm(self, algorithm_entry, params):
        """Initializes the algorithm."""
        algorithm = ALGORITHM_REGISTRY.get(algorithm_entry.name)(**params)
        return algorithm

    def _train_algorithm(self, algorithm):
    ...
    def _predict_and_postprocess_data(self, algorithm):
        """Makes predictions and performs post-processing."""
        X_pred = self._predict_and_postprocess(algorithm, self.test_data_in)
        return X_pred

    def evaluate_metrics(self, algorithm, X_pred):
        """Evaluates metrics for the algorithm."""
        for metric_entry in self.metric_entries:
            metric_cls = METRIC_REGISTRY.get(metric_entry.name)
            metric = self._initialize_metric(metric_cls, metric_entry)
            metric.calculate(self.test_data_out.binary_values, X_pred)
            self._metric_acc.add(metric, algorithm.identifier, metric.name)

    def _initialize_metric(self, metric_cls, metric_entry):
        """Initializes the metric."""
        if metric_entry.K is not None:
            return metric_cls(K=metric_entry.K)
        else:
            return metric_cls()